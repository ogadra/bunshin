...
_validate-stack stack:
    @if [ "{{stack}}" != "aws" ] && [ "{{stack}}" != "google-cloud" ]; then echo "Error: stack must be 'aws' or 'google-cloud', got '{{stack}}'"; exit 1; fi
...
init stack env: (_validate-stack stack) (_validate-env env) _validate-tf-backend-bucket
    terraform -chdir=terraform/{{stack}} init -reconfigure -backend-config="bucket=${TF_BACKEND_BUCKET}" -backend-config="key=bunshin/{{stack}}/{{env}}.tfstate"

# Plan changes for the specified environment
plan stack env: (_validate-stack stack) (_validate-env env)
    terraform -chdir=terraform/{{stack}} plan -var-file=environments/{{env}}.tfvars

# Apply changes for the specified environment
apply stack env: (_validate-stack stack) (_validate-env env)
    terraform -chdir=terraform/{{stack}} apply -var-file=environments/{{env}}.tfvars
...
destroy stack env: (_validate-stack stack) (_validate-env env)
    terraform -chdir=terraform/{{stack}} destroy -var-file=environments/{{env}}.tfvars
