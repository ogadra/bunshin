...
def region_stack(name: str, cidr: str, azs: str) -> dict[str, object]:
    with Cluster(name, graph_attr={**CLUSTER_FONT, "margin": "16"}):
        static = S3("S3 Static Assets")

        with Cluster(f"VPC {cidr}", graph_attr={**CLUSTER_FONT, "margin": "10"}):
            with Cluster(f"Private Subnets ({azs})", graph_attr={**CLUSTER_FONT, "margin": "18"}):
...
def main() -> None:
    with Diagram(
        ...
        apne1["static"] >> Edge(label="S3 replication") >> apne3["static"]

        accelerator >> Edge(label="weight 128") >> apne1["api_alb"]
        accelerator >> Edge(label="weight 128") >> apne3["api_alb"]

        apne1["nginx"] >> Edge(label="cross-region HTTPS", style="dashed") >> apne3["internal_alb"]
        apne3["nginx"] >> Edge(label="cross-region HTTPS", style="dashed") >> apne1["internal_alb"]
        apne1["private_dns"] >> Edge(label="VPC peering DNS", style="dashed") >> apne3["private_dns"]

        with Cluster("ECR", graph_attr={**CLUSTER_FONT, "margin": "24"}):
            ecr_nginx = ECR("bunshin/nginx")
            ecr_broker = ECR("bunshin/broker")
            ecr_runner = ECR("bunshin/runner")

        ecr_nginx >> Edge(label="image pull", style="dotted") >> apne1["nginx"]
        ecr_broker >> Edge(style="dotted") >> apne1["broker"]
        ecr_runner >> Edge(style="dotted") >> apne1["runner"]
        ecr_nginx >> Edge(label="replicated image pull", style="dotted") >> apne3["nginx"]
        ecr_broker >> Edge(style="dotted") >> apne3["broker"]
        ecr_runner >> Edge(style="dotted") >> apne3["runner"]
