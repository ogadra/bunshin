# AutopilotのpolicyがsecurityContext / capability drop / resource requestsをinjectするため、
# Deployment側では最小限しか書かない。broker Podは`kubernetes_service_account_v1.broker`のKSAを介した
# Workload IdentityでFirestoreに到達する
