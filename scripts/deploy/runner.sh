...
main() {
    local env_name="${1:?Usage: scripts/deploy/runner.sh <env> <aws_profile> <aws_account_id> <domain_name>}"
    local aws_profile_name="${2:?Usage: scripts/deploy/runner.sh <env> <aws_profile> <aws_account_id> <domain_name>}"
    local aws_account_id="${3:?Usage: scripts/deploy/runner.sh <env> <aws_profile> <aws_account_id> <domain_name>}"
    local domain_name="${4:?Usage: scripts/deploy/runner.sh <env> <aws_profile> <aws_account_id> <domain_name>}"
    local image_tag
    ...
    docker buildx build --platform "${PLATFORM}" "${tags[@]}" --push "${ROOT_DIR}/${SERVICE}"
