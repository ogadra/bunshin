...
apply_manifests() {
    local context="${1:?}"
    local defined_envs='$BROKER_GSA_EMAIL,$BROKER_KSA_NAME,$BROKER_REPLICAS,$BUNSHIN_STACKS,$DEPLOYER_EMAIL,$FIRESTORE_DATABASE,$FRAME_ANCESTORS,$GOOGLE_CLOUD_PROJECT,$IMAGE_TAG,$INTERNAL_DOMAIN,$INTERNAL_LB_NAME,$NGINX_NEG_NAME,$NGINX_REPLICAS,$NGINX_RESOLVER,$REGION,$REPOSITORY,$RUNNER_REPLICAS'
    local f

    # envsubst は未設定の変数を空に潰すだけなので、rollout 前にここで落とす
    : "${FRAME_ANCESTORS:?FRAME_ANCESTORS must be set (deploy/google-cloud/environments/<env>.env)}"
